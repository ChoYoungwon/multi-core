...
__global__ void matrixAdd(int * _a, int * _b, int * _c, int _size) {
    CudaTid tid(1);
    int tID = tid.get_tid_in_grid(1);
    if (tID < _size)
...
int main(int argc, char **argv)
{
    ...
    // 1D grid with 1D blocks (x_block & x_grid)
    int block_x = atoi(argv[1]);
    dim3 dimBlock(block_x, 1, 1);
    dim3 dimGrid(ceil((float)NUM_DATA / block_x), 1, 1);

    // 1D grid with 1D blocks (y_block & x_grid)
    // int block_y= atoi(argv[1]);
    // dim3 dimBlock(1, block_y, 1);
    // dim3 dimGrid(ceil((float)NUM_DATA / block_y), 1, 1);

    timer.onTimer(1);
    matrixAdd<<<dimGrid, dimBlock>>>(d_a, d_b, d_c, NUM_DATA);
    timer.offTimer(1);
